<!-- generated by scripts/generate-design-pack.ts from mcp-server/pack; edit there, not here; do not edit, run `pnpm gen:design-pack` -->
# Zandovi design elements (schema 1.0)

Field tables for `canvasData`, generated from the OpenAPI schema every service validates against. Types: `Name` = another table on this page; `a \| b` = enum or union; `T[]` = array; `T | null` = nullable. A field that is not required and shows a `default` may be omitted: the API fills it before validating. The API validates server-side and is authoritative; these tables are a guide. `unsupportedFields` lists schema-valid fields the renderer ignores.

## `CanvasDesign` (top level)

| Field | Type | Required | Notes |
|---|---|---|---|
| `schemaVersion` | 1.0 | yes |  |
| `viewport` | ViewportConfig | yes |  |
| `background` | BackgroundConfig | yes |  |
| `elements` | CanvasElement[] | yes | <= 500 items |

Element types: `rectangle`, `circle`, `ellipse`, `line`, `arrow`, `star`, `polygon`, `triangle`, `curvedRectangle`, `text`, `image`, `group`, `qrcode`, `barcode`, `icon`. Elements paint in array order (last on top). Origin: `x, y` is the top-left corner for every type except `circle`, `ellipse`, `triangle`, `star`, `polygon`, where it is the centre.

## `BaseElement` fields (every type except `group`)

| Field | Type | Required | Notes |
|---|---|---|---|
| `id` | string | yes |  |
| `x` | number | yes |  |
| `y` | number | yes |  |
| `width` | number | yes |  |
| `height` | number | yes |  |
| `rotation` | number | no | default 0 |
| `opacity` | number | no | 0..1, default 1 |
| `fill` | FillConfig | yes |  |
| `stroke` | string | no | default "#000000" |
| `strokeWidth` | number | no | >= 0, default 0 |
| `shadowColor` | string | no |  |
| `shadowBlur` | number | no |  |
| `shadowOffsetX` | number | no |  |
| `shadowOffsetY` | number | no |  |
| `draggable` | boolean | no | default true |
| `visible` | boolean | no | default true |
| `locked` | boolean | no | default false |
| `flipX` | boolean | no |  |
| `flipY` | boolean | no |  |

## `rectangle`

Axis-aligned rectangle with optional rounded corners; the workhorse for panels, backing shapes and dividers.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | rectangle | yes |  |
| `cornerRadius` | number | yes | >= 0 |

## `circle`

Circle centred on x,y with radius = width / 2; use for badges, dots and avatar masks.

Origin: centre. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | circle | yes |  |
| `radius` | number | yes | >= 0 |

## `ellipse`

Ellipse centred on x,y with radiusX = width / 2 and radiusY = height / 2; a stretched circle.

Origin: centre. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | ellipse | yes |  |
| `radiusX` | number | yes | >= 0 |
| `radiusY` | number | yes | >= 0 |

## `line`

Straight or polyline stroke through points (relative to x,y); height is normally 0 and colour comes from stroke.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | line | yes |  |
| `points` | number[] | yes |  |
| `lineCap` | butt \| round \| square | no | default "round"; butt is the plain end. |
| `lineJoin` | miter \| round \| bevel | no | default "round"; miter is the plain corner. |
| `dash` | number[] | no | default []; dash and gap lengths in px, e.g. [8, 4]; [] for a solid line. |

## `arrow`

Line with an arrowhead at its last point; pointerLength and pointerWidth size the head.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | arrow | yes |  |
| `points` | number[] | yes |  |
| `pointerLength` | number | no | >= 0, default 15 |
| `pointerWidth` | number | no | >= 0, default 15 |
| `lineCap` | butt \| round \| square | no | default "round"; butt is the plain end. |
| `lineJoin` | miter \| round \| bevel | no | default "round"; miter is the plain corner. |
| `dash` | number[] | no | default []; dash and gap lengths in px, e.g. [8, 4]; [] for a solid line. |

## `star`

Star centred on x,y with numPoints tips between outerRadius and innerRadius; for a regular N-gon use polygon.

Origin: centre. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | star | yes |  |
| `numPoints` | integer | yes | 3..100 |
| `innerRadius` | number | yes | >= 0; distance of the inner vertices from the centre; equal radii give a regular 2N-gon; for a regular N-gon use polygon. |
| `outerRadius` | number | yes | >= 0; distance of the tips from the centre, normally width / 2; equal radii give a regular 2N-gon; for a regular N-gon use polygon. |

## `polygon`

Regular N-gon centred on x,y with sides vertices on a circle of radius; use for hexagons, octagons and similar.

Origin: centre. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | polygon | yes |  |
| `sides` | integer | yes | 3..100; regular N-gon; hexagon = 6, octagon = 8. |
| `radius` | number | yes | >= 0 |

## `triangle`

Triangle centred on x,y filling the width by height box; variant picks the shape, points overrides it.

Origin: centre. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | triangle | yes |  |
| `variant` | right \| equilateral \| isosceles \| scalene | yes |  |
| `rightAnglePosition` | bottom-left \| bottom-right \| top-left \| top-right | no |  |
| `points` | number[] | no | exactly 6 items |

## `curvedRectangle`

Rectangle with one side bowed inward or outward by curveDepth; use for ticket stubs and wave dividers.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | curvedRectangle | yes |  |
| `curvedSide` | top \| right \| bottom \| left | yes |  |
| `curveType` | arc \| bezier | yes |  |
| `curveDepth` | number | yes | 0..100 |
| `curveDirection` | inward \| outward | yes |  |
| `cornerRadius` | number | yes | >= 0 |

## `text`

Text block wrapped inside width by height; fontFamily must be a bundled family and fill gives its colour.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: `textDecoration`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | text | yes |  |
| `text` | string | yes |  |
| `fontFamily` | string | yes |  |
| `fontSize` | number | yes | >= 1 |
| `fontStyle` | normal \| italic | no | default "normal" |
| `fontWeight` | integer | no | 100..900, default 400 |
| `textDecoration` | none \| underline \| line-through | no | default "none"; ignored by the export renderer (the designer canvas shows it); leave it at none. |
| `align` | left \| center \| right | no | default "left" |
| `verticalAlign` | top \| middle \| bottom | no | default "top" |
| `wrap` | word \| char \| none | no | default "word" |
| `ellipsis` | boolean | no | default false; legacy truncation flag; overflowMode wins when set, so leave it false unless overflowMode is ellipsis. |
| `lineHeight` | number | no | >= 0, default 1.2 |
| `letterSpacing` | number | no | default 0; extra spacing between characters in px; 0 for the font's default. |
| `autoFit` | boolean | no |  |
| `minFontSize` | number | no |  |
| `maxFontSize` | number | no |  |
| `overflowMode` | clip \| ellipsis \| visible | no |  |
| `ellipsisChar` | string | no |  |
| `textPadding` | TextPaddingPerSide | no |  |
| `isVariable` | boolean \| null | no |  |
| `variableName` | string \| null | no |  |
| `defaultValue` | string \| null | no |  |
| `variableLabel` | string \| null | no |  |
| `variableDescription` | string \| null | no |  |
| `variableRequired` | boolean \| null | no |  |
| `variableValidation` | VariableValidation \| null | no |  |
| `fontSlug` | string \| null | no | Stable directory-based font identifier (e.g. 'open-sans'). Optional; used alongside fontFamily for unambiguous font resolution. |

## `image`

Raster image from src fitted into width by height; isPlaceholder marks an image variable replaced at render time.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: `filters`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | image | yes |  |
| `src` | string | yes |  |
| `crop` | ImageCrop | no |  |
| `filters` | ImageFilters | no |  |
| `cornerRadius` | number | yes | >= 0 |
| `isPlaceholder` | boolean \| null | no |  |
| `variableName` | string \| null | no |  |
| `defaultSrc` | string \| null | no |  |
| `objectFit` | contain \| cover \| fill \| none | no |  |
| `validation` | ImageValidation | no |  |

## `group`

Container whose children are positioned relative to its x,y; it has no fill or stroke of its own.

Origin: top-left. Does not extend BaseElement (no fill/stroke); `children` are elements positioned relative to the group. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `id` | string | yes |  |
| `type` | group | yes |  |
| `name` | string | yes |  |
| `x` | number | yes |  |
| `y` | number | yes |  |
| `width` | number | yes |  |
| `height` | number | yes |  |
| `rotation` | number | no | default 0 |
| `opacity` | number | no | 0..1, default 1 |
| `draggable` | boolean | no | default true |
| `visible` | boolean | no | default true |
| `locked` | boolean | no | default false |
| `flipX` | boolean | no |  |
| `flipY` | boolean | no |  |
| `children` | CanvasElement[] | yes |  |
| `expanded` | boolean | yes |  |
| `scaleX` | number | no | default 1 |
| `scaleY` | number | no | default 1 |

## `qrcode`

QR code encoding data (or the variable value) in darkColor on lightColor; keep it square.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: `logoSrc`, `logoSize`, `logoPadding`.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | qrcode | yes |  |
| `data` | string | yes |  |
| `errorCorrectionLevel` | L \| M \| Q \| H | yes |  |
| `darkColor` | string | yes |  |
| `lightColor` | string | yes |  |
| `margin` | number | no |  |
| `logoSrc` | string | no |  |
| `logoSize` | number | no |  |
| `logoPadding` | number | no |  |
| `isVariable` | boolean \| null | no |  |
| `variableName` | string \| null | no |  |
| `defaultValue` | string \| null | no |  |
| `variableLabel` | string \| null | no |  |

## `barcode`

1D barcode of data in the given format with optional human-readable text; data must fit the format.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | barcode | yes |  |
| `data` | string | yes |  |
| `format` | CODE128 \| EAN13 \| UPC \| CODE39 \| ITF14 | yes |  |
| `lineColor` | string | yes |  |
| `background` | string | yes |  |
| `displayValue` | boolean | yes |  |
| `textAlign` | left \| center \| right | yes |  |
| `textPosition` | top \| bottom | yes |  |
| `fontSize` | number | yes | >= 1 |
| `fontFamily` | string | yes | generic CSS family such as monospace or sans-serif for the human-readable text; not checked against the bundled fonts. |
| `barWidth` | number | yes | >= 1 |
| `isVariable` | boolean \| null | no |  |
| `variableName` | string \| null | no |  |
| `defaultValue` | string \| null | no |  |
| `variableLabel` | string \| null | no |  |
| `fontSlug` | string \| null | no | Stable directory-based font identifier (e.g. 'open-sans'). Optional; used alongside fontFamily for unambiguous font resolution. |

## `icon`

Lucide icon named by iconName, drawn as strokes in iconColor and scaled to width by height.

Origin: top-left. Fields below are in addition to `BaseElement`. unsupportedFields: none.

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | icon | yes |  |
| `iconName` | string | yes |  |
| `iconColor` | string | yes |  |
| `iconStrokeWidth` | number | yes | >= 0 |

## Supporting types

### `ViewportConfig`

Design-space size in 96-DPI pixels plus unit, exportDpi and outputIntent; copy a preset rather than guessing.

| Field | Type | Required | Notes |
|---|---|---|---|
| `width` | integer | yes | 100..4096 |
| `height` | integer | yes | 100..4096 |
| `unit` | px \| mm \| cm \| in | no | default "px" |
| `exportDpi` | integer | no | 72..600, default 96 |
| `outputIntent` | screen \| print | no | default "screen" |

### `BackgroundConfig`

The canvas backdrop painted before every element: a solid colour, a gradient or an image with optional overlay.

One of: `SolidBackground`, `GradientBackground`, `ImageBackground` (discriminated by `type`).

### `SolidBackground`

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | solid | yes |  |
| `color` | string | yes |  |

### `GradientBackground`

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | gradient | yes |  |
| `gradient` | FillConfig | yes |  |

### `ImageBackground`

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | image | yes |  |
| `imageUrl` | string | yes |  |
| `fit` | cover \| contain \| stretch | no |  |
| `opacity` | number | no | 0..1 |
| `overlay` | object | no |  |

### `FillConfig`

Paint for an element or background gradient: solid colour, linear gradient or radial gradient; gradient points are 0..1 fractions.

One of: `SolidFill`, `LinearGradientFill`, `RadialGradientFill` (discriminated by `type`).

### `SolidFill`

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | solid | yes |  |
| `color` | string | yes |  |

### `LinearGradientFill`

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | linear | yes |  |
| `colorStops` | GradientColorStop[] | yes |  |
| `startPoint` | Point2D | yes | normalised 0..1 fractions of the painted box (element or canvas), not pixels; {x: 0, y: 0} is its top-left corner, {x: 1, y: 1} its bottom-right. |
| `endPoint` | Point2D | yes | normalised 0..1 fractions of the painted box, like startPoint; a top-to-bottom gradient runs from {x: 0, y: 0} to {x: 0, y: 1}. |

### `RadialGradientFill`

| Field | Type | Required | Notes |
|---|---|---|---|
| `type` | radial | yes |  |
| `colorStops` | GradientColorStop[] | yes |  |
| `centerPoint` | Point2D | yes | normalised 0..1 fractions of the painted box (element or canvas); {x: 0.5, y: 0.5} is its centre. |
| `radius` | number | yes | 0..1 fraction of the larger of the painted box's width and height, not pixels; 0.5 from the centre reaches the edge along the longer side. |

### `GradientColorStop`

| Field | Type | Required | Notes |
|---|---|---|---|
| `offset` | number | yes | 0..1 |
| `color` | string | yes |  |

### `Point2D`

| Field | Type | Required | Notes |
|---|---|---|---|
| `x` | number | yes |  |
| `y` | number | yes |  |

### `TextPaddingPerSide`

| Field | Type | Required | Notes |
|---|---|---|---|
| `top` | number | yes | >= 0 |
| `right` | number | yes | >= 0 |
| `bottom` | number | yes | >= 0 |
| `left` | number | yes | >= 0 |

### `VariableValidation`

| Field | Type | Required | Notes |
|---|---|---|---|
| `required` | boolean | no |  |
| `minLength` | number | no |  |
| `maxLength` | number | no |  |
| `min` | number | no |  |
| `max` | number | no |  |
| `pattern` | string | no |  |
| `format` | string | no |  |
| `options` | string[] | no |  |

### `ImageCrop`

| Field | Type | Required | Notes |
|---|---|---|---|
| `x` | number | yes |  |
| `y` | number | yes |  |
| `width` | number | yes |  |
| `height` | number | yes |  |

### `ImageFilters`

| Field | Type | Required | Notes |
|---|---|---|---|
| `brightness` | number | yes |  |
| `contrast` | number | yes |  |
| `saturation` | number | yes |  |
| `blur` | number | yes |  |
| `grayscale` | boolean | yes |  |
| `sepia` | boolean | yes |  |

### `ImageValidation`

| Field | Type | Required | Notes |
|---|---|---|---|
| `minWidth` | number | no |  |
| `minHeight` | number | no |  |
| `maxWidth` | number | no |  |
| `maxHeight` | number | no |  |
| `aspectRatio` | string | no |  |
| `allowedFormats` | jpeg \| png \| webp \| gif[] | no |  |
| `maxFileSize` | number | no |  |
