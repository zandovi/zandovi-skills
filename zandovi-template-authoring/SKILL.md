...
<!-- generated:counts -->
- `references/guide.md`: authored gotchas (coordinate origins, fill vs color, z-order, variables, presets) and the available tools
- `references/codes.md`: the 29 validator codes with their severity and meaning, generated from the rule catalogue
...
3. **Write the JSON.** Copy the structure of the closest design in `references/examples.md`. Omit fields that are at their default and set them only when they differ (the defaults table is in the `Field names` section of `references/guide.md`); the API fills the rest before validating. Check coordinate origins for `circle`, `ellipse`, `triangle`, `star` and `polygon` (centre origin) before placing them. Order the `elements` array back to front: background shapes, images, decorative overlays, text last.
